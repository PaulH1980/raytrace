...
namespace RayTrace
{
    ...
    void SampledSpectrum::Init()
    {
        ...
        for (int i = 0; i < nSpectralSamples; ++i) {
            float wl0 = Lerp( sampledLambdaStart, sampledLambdaEnd, float(i) / float(nSpectralSamples));
            float wl1 = Lerp( sampledLambdaStart, sampledLambdaEnd, float(i + 1) / float(nSpectralSamples));
    ...
    SampledSpectrum SampledSpectrum::FromRGB(const float rgb[3],
        eSpectrumType type) {
        SampledSpectrum r;
        if (type == eSpectrumType::SPECTRUM_REFLECTANCE) {
    ...
    SampledSpectrum SampledSpectrum::FromXYZ(const float xyz[3], eSpectrumType type /*= SpectrumType::Reflectance*/)
    {
        float rgb[3];
        XYZToRGB(xyz, rgb);
    ...
    SampledSpectrum::SampledSpectrum(const RGBSpectrum& r, eSpectrumType t) {
        float rgb[3];
        r.ToRGB(rgb);
    ...
    SampledSpectrum SampledSpectrum::FromSampled(const float* lambda, const float* v, int n)
    {
        ...
        for (int i = 0; i < nSpectralSamples; ++i) {
            ...
            float lambda0 = Lerp(sampledLambdaStart, sampledLambdaEnd, float(i) / float(nSpectralSamples));
            float lambda1 = Lerp(sampledLambdaStart, sampledLambdaEnd, float(i + 1) / float(nSpectralSamples));
    ...
    float InterpolateSpectrumSamples(const float* lambda, const float* vals, int n,
        ...
        return Lerp( vals[offset], vals[offset + 1], t);
    }
    ...
    void ResampleLinearSpectrum(const float* lambdaIn, const float* vIn, int nIn,
        ...
        float* vOut) {
        assert(nOut >= 2);
        for (int i = 0; i < nIn - 1; ++i) assert(lambdaIn[i + 1] > lambdaIn[i]);
        ...
        auto lambdaInClamped = [&](int index) {
            ...
            if (index == -1) {
                assert((lambdaMin - delta) < lambdaIn[0]);
                return lambdaMin - delta;
            }
            else if (index == nIn) {
                assert((lambdaMax + delta)> lambdaIn[nIn - 1]);
        ...
        auto vInClamped = [&](int index) {
           assert(index >= -1 && index <= nIn);
            return vIn[std::clamp(index, 0, nIn - 1)];
        };
        ...
        auto resample = [&](float lambda) -> float {
            ...
            if (end - start == 2 && lambdaInClamped(start) <= lambda - delta &&
                ...
                assert(t >= 0 && t <= 1);
                return Lerp( vInClamped(start), vInClamped(end), t);
            }
            else {
        ...
        for (int outOffset = 0; outOffset < nOut; ++outOffset) {
            ...
            float lambda =
                Lerp( lambdaMin, lambdaMax, float(outOffset) / (nOut - 1));
            vOut[outOffset] = resample(lambda);
        }
    }

    RGBSpectrum::RGBSpectrum(const RGBSpectrum& s, eSpectrumType type /*= SpectrumType::Reflectance*/)
    {
        *this = s;
    }
    ...
    RGBSpectrum::RGBSpectrum(float v /*= 0.f*/) : CoefficientSpectrum<3>(v)
    {

    }

    RGBSpectrum RGBSpectrum::FromRGB(const float rgb[3], eSpectrumType type /*= SpectrumType::Reflectance*/)
    {
        RGBSpectrum s;
        s.c[0] = rgb[0];
        s.c[1] = rgb[1];
    ...
    void RGBSpectrum::ToXYZ(float xyz[3]) const
    {
        RGBToXYZ(c, xyz);
    }

    RGBSpectrum RGBSpectrum::FromXYZ(const float xyz[3], eSpectrumType type /*= SpectrumType::Reflectance*/)
    {
        RGBSpectrum r;
        XYZToRGB(xyz, r.c);
        return r;
