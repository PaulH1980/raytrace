...
namespace RayTrace {
    ...
    static thread_local uint64_t tokenizerMemory = 0;
...
#if defined(PBRT_HAVE_MMAP) || defined(IS_WINDOWS)
            ...
            if (type == PARAM_TYPE_INT) {
                ...
                if ((nItems % 2) != 0)
                    Warning(
                        ...
                        item.name.c_str());
                std::unique_ptr<Vector2f[]> pts(new Vector2f[nItems / 2]);
                ...
                std::unique_ptr<Vector3f[]> pts(new Vector3f[nItems / 3]);
                ...
                if ((nItems % 3) != 0)
                    Warning(
                        ...
                        "Ignoring last %d of them.",
                        item.name.c_str(), nItems % 3);
                std::unique_ptr<Vector3f[]> normals(new Vector3f[nItems / 3]);
                for (int j = 0; j < nItems / 3; ++j) {
        ...
        while (true) {
            ...
            case 'A':
                if (tok == "AttributeBegin")
                    ...
                    basicParamListEntrypoint(eSpectrumType::SPECTRUM_ILLUMINANT,
                        pbrtAreaLightSource);
                else if (tok == "Accelerator")
                    basicParamListEntrypoint(eSpectrumType::SPECTRUM_REFLECTANCE,
                        pbrtAccelerator);
                else
                    syntaxError(tok);
                break;
            ...
            case 'C':
                if (tok == "ConcatTransform") {
                    ...
                    basicParamListEntrypoint(eSpectrumType::SPECTRUM_REFLECTANCE, pbrtCamera);
                else
                    syntaxError(tok);
                break;
            ...
            case 'F':
                if (tok == "Film")
                    basicParamListEntrypoint(eSpectrumType::SPECTRUM_REFLECTANCE, pbrtFilm);
                else
                    syntaxError(tok);
                break;
            ...
            case 'S':
                if (tok == "Shape")
                    basicParamListEntrypoint(eSpectrumType::SPECTRUM_REFLECTANCE, pbrtShape);
                else if (tok == "Sampler")
                    basicParamListEntrypoint(eSpectrumType::SPECTRUM_REFLECTANCE,
                        pbrtSampler);
                else if (tok == "Scale") {
            ...
            case 'T':
                if (tok == "TransformBegin")
                    ...
                    basicParamListEntrypoint(
                        eSpectrumType::SPECTRUM_REFLECTANCE,
