...
namespace RayTrace {
    ...
    void ClipDistanceState::applyGL() const
    {
    ...
    voidAlphaStateDepr::applyGL() const
    {
        glAlphaFunc(mode, refvalue);
    }

    void AlphaStateDepr::getGL()
    {
    ...
    void StencilState::applyGL() const
    {
    ...
    void DepthState::applyGL() const
    {
        glDepthFunc(func);
    }

    void DepthState::getGL()
    {
    ...
    void LogicState::applyGL() const
    {
        glLogicOp(op);
    }

    void LogicState::getGL()
    {
    ...
    void RasterState::applyGL() const
    {
...
#if STATESYSTEM_USE_DEPRECATED
    void RasterStateDepr::applyGL() const
    {
        glLineStipple(lineStippleFactor, lineStipplePattern);
        glShadeModel(shadeModel);
    }

    void RasterStateDepr::getGL()
    {
        GLint pattern;
    ...
    void PrimitiveState::applyGL() const
    {
    ...
    void SampleState::applyGL() const
    {
        glSampleCoverage(coverage, invert);
        glSampleMaski(0, mask);
    }

    void SampleState::getGL()
    {
    ...
    //////////////////////////////////////////////////////////////////////////
    /*
    void ViewportState::applyGL() const
    {
      if (useSeparate){
    ...
    */
    ...
    void DepthRangeState::applyGL() const
    {
        if (useSeparate) {
    ...
    //////////////////////////////////////////////////////////////////////////
    /*
    void ScissorState::applyGL() const
    {
      if (useSeparate){
    ...
    */
    ...
    void ScissorEnableState::applyGL() const
    {
        if (separateEnable) {
            for (GLuint i = 0; i < MAX_VIEWPORTS; i++) {
    ...
    void MaskState::SetColorMask(int _bufferIdx, bool _r, bool _g, bool _b, bool _a)
    {
        colormask[_bufferIdx][0] = _r;
        colormask[_bufferIdx][1] = _g;
        colormask[_bufferIdx][2] = _b;
        colormask[_bufferIdx][3] = _a;
    }

    //////////////////////////////////////////////////////////////////////////

    void MaskState::applyGL() const
    {
    ...
    void FBOState::applyGL(bool skipFboBinding) const
    {
        if (!skipFboBinding) {
    ...
    void VertexEnableState::applyGL(GLbitfield changed) const
    {
    ...
    void VertexFormatState::applyGL(GLbitfield changedFormat, GLbitfield changedBinding) const
    {
        for (GLuint i = 0; i < MAX_VERTEXATTRIBS; i++) {
    ...
    void VertexImmediateState::applyGL(GLbitfield changed) const
    {
        for (GLuint i = 0; i < MAX_VERTEXATTRIBS; i++) {
    ...
    void VertexImmediateState::getGL()
    {
        for (GLuint i = 0; i < MAX_VERTEXATTRIBS; i++) {
            switch (data[i].mode) {
            case VERTEXMODE_FLOAT:
    ...
    void ProgramState::applyGL() const
    {
        glUseProgram(program);
    }

    void ProgramState::getGL()
    {
    ...
    // keep in sync!
    static GLenum s_stateEnums[NUM_STATEBITS] = {
      GL_BLEND,
      GL_COLOR_LOGIC_OP,
      GL_CULL_FACE,
      GL_DEPTH_CLAMP,
    ...
    void EnableState::applyGL(GLbitfield changedBits) const
    {
    ...
    //////////////////////////////////////////////////////////////////////////
#if STATESYSTEM_USE_DEPRECATED
    static GLenum s_stateEnumsDepr[NUM_STATEBITSDEPR] = {
      GL_ALPHA_TEST,
      GL_LINE_STIPPLE,
      GL_POINT_SMOOTH,
      GL_POINT_SPRITE,
    ...
    void EnableStateDepr::applyGL(GLbitfield changedBits) const
    {
    ...
    RenderState::RenderState() : basePrimitiveMode(GL_TRIANGLES)
        , boundVAO(INVALID_ID)
    {
    ...
    void RenderState::applyGL(bool coreonly, bool skipFboBinding) const
    {
        enable.applyGL(); 
#if STATESYSTEM_USE_DEPRECATED
    ...
    void RenderState::getGL(bool coreonly)
    {
        enable.getGL();
#if STATESYSTEM_USE_DEPRECATED
        if (!coreonly) enableDepr.applyGL();
    ...
    void StateSystem::set(StateID id, const RenderState& state, GLenum basePrimitiveMode)
    {
    ...
    const RenderState& StateSystem::get(StateID id) const
    {
    ...
    void StateSystem::applyDiffGL(const StateDiff& diff, const RenderState& state, bool skipFboBinding)
    {
    ...
    void StateSystem::makeDiff(StateDiff& diff, const StateInternal& fromInternal, const StateInternal& toInternal) const
    {
        const RenderState& from = fromInternal.state;
        const RenderState& to = toInternal.state;
        makeDiff(diff, from, to);
    }

    void StateSystem::makeDiff(StateDiff& diff, const RenderState& from, const RenderState& to) const
    {
